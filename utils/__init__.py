...
def _lookup_identifier_method():
    ...
    try:
        ...
        raise ImportError("Unable to import module '%s' provided for HAYSTACK_IDENTIFIER_METHOD." % module_path)

    identifier_method = getattr(module, method_name, None)

    if not identifier_method:
        raise AttributeError(
            "Provided method '%s' for HAYSTACK_IDENTIFIER_METHOD does not exist in '%s'." % (method_name, module_path)
        )
